...
# Write data.js with retry: a transient lock (OneDrive sync, AzCopy from a prior run, or an
# overlapping manual run) must not abort the whole export. ~6s of retries clears normal locks.
for ($try = 1; $try -le 8; $try++) {
    try { Set-Content -Path $OutFile -Value $content -Encoding UTF8 -ErrorAction Stop; break }
    catch {
        if ($try -eq 8) { throw }
        Log-Export ("data.js locked, retry $try/8: " + $_.Exception.Message)
