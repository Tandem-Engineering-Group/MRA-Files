...
P_PROJECT,P_TASKID,P_PHASE,P_TYPE = 'field_1','field_2','field_3','TaskType'
...
def main():
    ...
    ap.add_argument('--asof',  default='')   # when the lists.json SNAPSHOT was written (blob lastModified) - the true freshness of the board rows
    args = ap.parse_args()
    ...
    payload = {
        ...
        'holidays':      holidays,
        'source':        'lists',
        # generatedAt says when the EXPORT ran; listsAsOf says when the lists SNAPSHOT it was built
        # from was taken. The dashboard holds optimistic edits until listsAsOf passes the edit time -
        # a fresh export built from a pre-edit snapshot must not release them (edits looked "reverted").
        'listsAsOf':     (args.asof.strip() or None),
    }
    if not payload['listsAsOf']:
        del payload['listsAsOf']
