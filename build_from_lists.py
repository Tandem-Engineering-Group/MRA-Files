...
def main():
    ...
    # --- TEMP DEBUG (remove after): real internal column names present on Project Tasks,
    # so we can find the live 'Type' column's true internal name (it isn't 'Type' or field_4). ---
    _pkey_counts = {}
    for it in proj_in:
        for k in it.keys():
            _pkey_counts[k] = _pkey_counts.get(k, 0) + 1
    # sample values for the unmapped field_N columns, to spot which one is 'Type'
    _pkey_samples = {}
    for k in ('field_4','field_15','field_16','field_17','field_18','field_19','field_20','field_21','Type'):
        vals = []
        for it in proj_in:
            v = it.get(k)
    ...
    payload = {
        ...
        'holidays':      holidays,
        'source':        'lists',
        '_pkeyCounts':   _pkey_counts,   # TEMP debug — internal column names on Project Tasks
        '_pkeySamples':  _pkey_samples,  # TEMP debug — sample values of unmapped columns
    }
